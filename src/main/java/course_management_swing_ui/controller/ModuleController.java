...
import course_management_swing_ui.model.CompulsoryModule;
import course_management_swing_ui.model.ElectiveModule;
import course_management_swing_ui.model.Module;
import course_management_swing_ui.repository.ModuleRepositoryImpl;
...
import course_management_swing_ui.view.module.NewModuleView;
import course_management_swing_ui.view.module.ListModuleView;
 ...
 * @Overview Represents the controller component, which handles the user interaction as well as logic business of Module
 * entities
 * @attributes <pre>
 *   View       View
 * </pre>
...
public class ModuleController extends BaseController {
                    ...
                    if (result == JOptionPane.YES_OPTION) {
                        ...
                        for (int i = tblModules.getRowCount() - 1; i >= 0; i--) {
                            ...
                            if (delete) {
                                ...
                                modules.add(moduleRepository.findById(code));
                            }
                        }
    ...
    private boolean addModule(String name, int semester, int credits, Module.ModuleType mt, String department) {
        if (Module.validateName(name) && Module.validateSemester(semester) && Module.validateCredits(credits) && Module.validateModuleType(mt)) {
            try {
                moduleRepository.add(ModuleFactory.getInstance().createModule(name, semester, credits, mt, department));
                fetchData();
    ...
    public static void fetchData() {
        ...
        moduleDbContext.addAll(moduleRepository.findAll());
